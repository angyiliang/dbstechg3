...
// public facing function to start  plotting rainfall graphs
function get_rain_fall(startDate, endDate, hlevel, showMonth, customData=null){

    var start = moment(startDate);
    var end = moment(endDate);

    $.getJSON("./data/rainfall.json", function(json) {
        
        compare_disease_rainfall(start, end, hlevel, json, showMonth, customData);
        
    });
}

function compare_disease_rainfall(startDate, endDate, hlevel, rain_data, showMonth, customData){
    
    var start = moment(startDate);
    var end = moment(endDate);
    var compiledData;
    var compiledRain = CompileRainFall(rain_data, start, end, showMonth);
    
    let data = $.getJSON("./data/sort_by_date.json", function(json) {
        
        
        if( hlevel == 0){
            compiledData = CompileAllDiseasesForRainfall(json, start, end, showMonth, customData);
        } else if (hlevel == 1){
            compiledData = CompileDiseaseCategoryForRainfall(json, start, end, showMonth, customData);
        } else if (hlevel == 2){
            compiledData = CompileDiseaseNameForRainfall(json, start, end, showMonth, customData);
        }

        reload_rain_graph(start, end, compiledData, compiledRain, showMonth);
    });
}
...
function reload_rain_graph(startDate, endDate, compiledData, compiledRain, showMonth){
    ...
    $("#graph-container").append(`<div id="rainfall-container" style="width: 75%">
    <canvas id="rainfall-canvas"></canvas>
</div>
...
<div id="disease-container" style="width: 75%">
    ...
    var rain_ctx = document.getElementById("rainfall-canvas").getContext("2d");
    ...
    var rainChart = new Chart(rain_ctx, {
        type: 'line',
        data: {
          datasets: rainGraphData,
          labels: selectedMonths
        },
        options: option
    });
...
function CompileRainFall(jsonData, start, end){
    var compiled = [];
...
function CompileAllDiseasesForRainfall(jsonData, start, end, showMonth, customData){
    var compiled = {};
    var years = [];
    var months = [];
    var highest = 0;
...
function CompileDiseaseCategoryForRainfall(jsonData, start, end, showMonth, customData){
    var compiled = {};
    var years = [];
    var months = [];
    var highest = 0;
...
function CompileDiseaseNameForRainfall(jsonData, start, end, showMonth, customData){
    var compiled = {};
    var years = [];
    var months = [];
    var highest = 0;
